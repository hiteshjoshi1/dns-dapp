...
contract EthereumDIDRegistry {
    ...
    modifier onlyOwner(address identity, address actor) {
        require (actor == identityOwner(identity),"actor is not owner");
        _;
    }
    ...
    function checkSignature(address identity, uint8 sigV, bytes32 sigR, bytes32 sigS, bytes32 hash) internal returns(address) {
        ...
        require(signer == identityOwner(identity),"Signature not of owner");
        nonce[signer]++;
        return signer;
    }
    ...
    // 2: byte(0) - the version byte as defined in EIP 191, we use 0
    // 3: this - the validator address in our case this contract address
    // 4: followed by nonce, identity address, method called, method params
     */
