...
export class NameReserveComponent {
  ...
  createForm() {
    this._reserveForm = this.formBuilder.group({
      ...
      reservationFee: ["", Validators.compose([Validators.required])]
    });
  }
  public reserveName() {
    ...
    this.nameService
      .reserveName(name, String(fee))
      .then(res => {
        if (res)
          this._modal._displayResult = ` ${name} reservation request sent to Blockchain!`;
